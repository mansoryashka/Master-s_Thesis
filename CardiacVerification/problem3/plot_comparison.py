...
if __name__ == '__main__':
    ...
    ax2.plot(fem_x[-6:-2], fem_z[-6:-2])
    ax3.plot(fem_x[:5], fem_z[:5])
    ...
    for lr, nn, nl, i in zip([0.1, 0.1, 0.05], [40, 50, 50], [5, 3, 3], [1, 2, 3]):
        ...
        energy = GuccioneTransverseActiveEnergyModel(C, bf, bt, bfs, kappa=1E3)
        DemLV = DeepEnergyMethodLV(model, energy)
        DemLV.model.load_state_dict(torch.load(f'trained_models/model_{lr}_nn{nn}_nl{nl}'))
...
#     fig.tight_layout()
    ...
    fig.savefig('figures/line_plot300.pdf')
    # plt.show()